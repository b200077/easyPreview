...
Imports Microsoft.NET.StringTools
...
Module NamedPipeUtils
    ...
    Public Function ListNamedPipes(Optional prefix As String = "") As List(Of String)
        ...
        Dim findData As WIN32_FIND_DATA = New WIN32_FIND_DATA()
...
Partial Public Class Form1
    ...
    Public doc As Spire.Pdf.PdfDocument = New Spire.Pdf.PdfDocument()
    ...
    Private Sub delete_thumbnail(curitem As String)
        ...
        psi.Arguments = "/c nconvert -out png -q 50 -o """ & preImagePath & """ """ & curitem & """"
    ...
    Async Function ProcessItemsAsync(items As List(Of String), removeFromStart As Boolean) As Task(Of List(Of String))
        Return Await Task.Run(Function()
                                  ...
                                  For Each i In indices
                                      ...
                                      Dim finalName As String = If(isLabel, "★" & standardized, standardized)
    ...
    Public Async Function read_text_data(oripath As String, targetList As ListBox, Optional ignoreReading As Boolean = False) As Task
        ...
        If oripath.Contains(".txt") Then
            rgx = New Regex("(htp.*)")
        End If
        Dim operate_rgx As Regex = New Regex("^(\[.*\])")
    ...
    Private Sub Find_Episode(sender As Object, e As EventArgs)
        If FileCollection.SelectedItem Is Nothing Then
            Return
        End If
        Dim rgx As Regex = New Regex("(.*\W*)\W(\d{1,2})(\W.*)")
        ...
        Using fs As New FileStream(filePath, FileMode.Open, FileAccess.Read)
            Using br As New BinaryReader(fs)
                ...
                If Not signature.SequenceEqual(pngSignature) Then
                    Return Windows.Size.Empty ' 文件不是有效的 PNG
                End If
    ...
    Private Sub OptionBox_CheckedChanged(sender As Object, e As EventArgs)
        ...
        Else
            Try
                ComboBox2.Text = Path.GetFileNameWithoutExtension(FileCollection.SelectedItems(FileCollection.SelectedItems.Count - 1).ToString)
            Catch
            End Try
        End If
        Form1_scale()
    ...
    Public Function SearchAllSubDirectories() As Boolean
        Return If(CheckBox3.Checked, True, False)
    End Function
    ...
    Public Sub Next_item()
        ...
        targetList.SelectedIndex = targetList.SelectedIndex + 1
        targetList.SetSelected(targetList.SelectedIndex, False)
    End Sub
    ...
    'Dim asmrSubnailKeyword As String() = {"メイン", "表紙", "ロゴあり", "高画質パッケージ", "ジャケット", "イラスト"}
    Public Sub FindFirstBitmap(tarlist As ListBox)
        'If isopening Then Return
        'Dim data As String() = searchfiles_with_multiple_situation(tarlist.Items.Cast(Of String).ToArray, {"Images", "!其他圖片"}, asmrSubnailKeyword)
    ...
    Public Function getMovedFolder(curitem As String)
        ...
        Console.WriteLine("last_folder = desk " & last_folder = desk)
    ...
    Public Async Sub file_Preview(checkFileSelect As Boolean)
        ...
        If Not Ext_judge(curitem, "網頁") AndAlso Not Ext_judge(curitem, "目錄") Then
            ...
            installs = New String() {ComboBox2.Text, last_write, "刪去第N個字元 string:n"}
        ...
        ElseIf Ext_judge(curitem, "目錄") Then
            ...
            Try
                ...
                installs = New String() {ComboBox2.Text, createTime, "Custom"}
            Catch
            End Try
        ElseIf Ext_judge(curitem, "網頁") Then
            Try
                ComboBox2.Text = Path.GetFileNameWithoutExtension(curitem)
                installs = New String() {ComboBox2.Text}
            Catch
            End Try
        End If
        ...
        If Ext_judge(curitem, "目錄") Then
            ...
            GroupBox3.Visible = If(curitem.Contains("テグラユウキ"), True, False)
            If CheckedListBox1_isCheck("ASMR播放模式") Then
    ...
    Public Async Sub OnButton4Click(sender As Object, e As EventArgs) Handles Button4.Click
        ...
        Select Case control
            ...
            Case "SpecialButton"
                Select Case ComboBox7.Text
                    ...
                    '    FileCollection.Items.Cast(Of String).ToList.ForEach(Sub(i) folder1.Add(i.Replace(PathComboBox.Text, "")))
                    '    SubFileCollection.Items.Cast(Of String).ToList.ForEach(Sub(i) folder2.Add(i.Replace(TargetComboBox.Text, "")))
                    ...
                    '        My.Computer.FileSystem.CopyFile(PathComboBox.Text & file, TargetComboBox.Text & Path.DirectorySeparatorChar & filename, UIOption.AllDialogs)
                    '    Next
                    '    'folder2 to folder1
                    '    For Each file In addToFolder1
                    '        Dim filename As String = Path.GetFileName(file)
                    '        My.Computer.FileSystem.CopyFile(TargetComboBox.Text & file, PathComboBox.Text & Path.DirectorySeparatorChar & filename, UIOption.AllDialogs)
                    '    Next
                    Case "檢查路徑"
                        For Each foundfile As String In collections
                            counter += 1
                    ...
                    Case "檢查網址"
                        ...
                        For Each foundfile As String In collections
                            counter += 1
                            Dim checkIndex = collections_number(counter)
                            FileCollection.Items(checkIndex) = standardized_denomination(FileCollection.Items(checkIndex))
                        Next
                    Case "加標籤"
                        If targetList IsNot FileCollection Then Return
                        For Each foundfile As String In collections
        ...
        If Not CheckBox5.Checked And Not targetList Is SubFileCollection And FileCollection.Items.Count > 0 Then
            ...
            End If
        End If
        If CheckBox5.Checked = True And Not ComboBox7.Text = "sendtoDL" Then OpenButton.Checked = True
        refresh_backup()
...
Or NotifyFilters.DirectoryName),
    ...
    Private Sub Quick_Rename(sender As Object, e As KeyEventArgs) Handles ComboBox2.KeyDown, TextBox3.KeyDown
